...
class StreamingManager:
    ...
    def _initialize_components(self):
        ...
        parser = SQLParser(self.config.tracker.sql_query, self.config.tracker.base_table)
        join_graph = parser.get_join_graph()
        self.dependency_graph = DependencyGraph(self.config.tracker.base_table, join_graph)
    ...
    def start_streaming(self, blocking: bool = True) -> Optional[threading.Thread]:
        ...
        logger.info(
            f"Slot: {self.config.replication.slot_name}, Plugin: {self.config.replication.plugin}"
        )
        ...
        self.consumer = ReplicationConsumer(
            self.config.replication, self.routing_engine, tables=tables
        )
        ...
        if blocking:
            ...
            self._streaming_thread = threading.Thread(
                target=self.consumer.start_streaming, daemon=True, name="cdc-streaming"
            )
...
def start_cdc_streaming(
    config: Config, verbose: bool = False, blocking: bool = True
) -> StreamingManager:
    """
