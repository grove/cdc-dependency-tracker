...
@click.option(
    '--once',
    is_flag=True,
    help='Run percolation once and exit (instead of continuous loop)'
)
@click.option(
    '--cleanup',
    is_flag=True,
    help='Clean up old percolated items after each batch'
)
@click.option(
    '--verbose',
    '-v',
...
def main(config: str, once: bool, cleanup: bool, verbose: bool):
    ...
    try:
        ...
        logger.info(f"Starting percolator (interval={interval}s, batch_size={cfg.tracker.percolation_batch_size})")
...
def run_percolation_cycle(percolator: PercolationEngine, cleanup: bool) -> None:
    ...
    print(f"items_percolated={result['items_percolated']} "
