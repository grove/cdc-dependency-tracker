        ...
        if isinstance(condition, exp.EQ):
            ...
            if isinstance(left, exp.Column) and isinstance(right, exp.Column):
                ...
                if left_table and right_table:
                    self.joins.append(
                        JoinCriteria(left_table, right_table, left_col, right_col)
                    )
    ...
    def _extract_base_id_column(self) -> Optional[str]:
        ...
        for select in self.parsed.find_all(exp.Select):
            for projection in select.expressions:
                # Check if this is an aliased column (e.g., c._id as customer_id)
                if isinstance(projection, exp.Alias):
                    column = projection.this
                    if isinstance(column, exp.Column):
                        if str(column.table) == base_alias and \
                           str(column.name) in ('_id', 'id'):
                            return str(projection.alias)
                # Check if this is an unaliased column (e.g., c._id)
                elif isinstance(projection, exp.Column):
                    if str(projection.table) == base_alias and \
                       str(projection.name) in ('_id', 'id'):
