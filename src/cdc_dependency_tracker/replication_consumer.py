...
class ReplicationConsumer:
    ...
    def connect(self) -> None:
        ...
        try:
            logger.info(f"Connecting to {self.config.host}:{self.config.port}/{self.config.dbname} for replication")
            ...
            logger.info(f"Connected to replication slot '{self.config.slot_name}' with publication '{self.config.publication_name}'")
    ...
    def _ensure_slot_exists(self) -> None:
        ...
        try:
            ...
            if check_cur.fetchone():
                ...
                logger.info(f"Creating replication slot '{self.config.slot_name}' with plugin 'pgoutput'")
                check_cur.execute(
        ...
        try:
            ...
            if check_cur.fetchone():
                ...
                if self.tables:
                    ...
                    logger.info(f"Creating publication '{self.config.publication_name}' for tables: {table_list}")
                    check_cur.execute(
                        f"CREATE PUBLICATION {self.config.publication_name} FOR TABLE {table_list}"
                    )
                else:
                    logger.info(f"Creating publication '{self.config.publication_name}' for ALL TABLES")
                    check_cur.execute(
        ...
        while not shutdown_requested and retry_count < self.max_retries:
            try:
                ...
                if retry_count < self.max_retries:
                    logger.info(f"Retrying in {self.retry_delay}s... (attempt {retry_count}/{self.max_retries})")
        ...
        try:
            for message in self.reader:
                ...
                try:
                    ...
                    logger.debug(f"Processed event: table={event.table}, op={event.operation} -> {result}")
                    message_count += 1
