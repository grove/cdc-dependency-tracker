...
class RoutingEngine:
    ...
    def _handle_depth_1(self, event: CDCEvent) -> ProcessingResult:
        ...
        if event.is_create:
            # INSERT or snapshot
            parent_id = event.after.get(parent_id_col)
            if parent_id:
        ...
        if base_entities:
            ...
            logger.info(f"Tracked {count} {self.graph.base_table} entities for {event.table} change (immediate)")
    ...
    def _handle_depth_2_adaptive(self, event: CDCEvent) -> ProcessingResult:
        ...
        if event.is_delete:
            parent_row_id = event.before.get(join_col)
        else:
            parent_row_id = event.after.get(join_col)
        ...
        try:
            ...
            if parent_table == self.graph.base_table:
                ...
                logger.info(f"Tracked {self.graph.base_table} {parent_row_id} for {event.table} change (immediate)")
                return ProcessingResult(immediate=1, queries=0)
            else:
                ...
                if grandparent_info:
                    ...
                    base_entity_id = self.db.query_parent_entity(
                        parent_table, 
                        grandparent_join_col, 
                        parent_row_id,
                        "_deleted = FALSE"
                    )
                    if base_entity_id:
                        self.db.track_entity(base_entity_id, self.base_table_id_column)
                        logger.info(f"Tracked {self.graph.base_table} {base_entity_id} for {event.table} change (immediate)")
                        return ProcessingResult(immediate=1, queries=1)
                    else:
                        logger.warning(f"No {self.graph.base_table} found for {parent_table}={parent_row_id}")
