"""End-to-end tests for customers example with function-scoped CDC lifecycle.

Each test creates its own replication slot and publication, uses unique IDs,
and filters events to avoid cross-test contamination.
"""
...
from cdc_dependency_tracker.config import Config, DatabaseConfig
...
TABLES_UNDER_TEST = ("customers", "orders", "order_lines", "products")

CDC_CONSUMER_STARTUP_TIMEOUT_SECONDS = 15
CDC_CONSUMER_STOP_TIMEOUT_SECONDS = 10
CDC_EVENT_TIMEOUT_SECONDS = 10.0
CONTAINER_DB_READY_TIMEOUT_SECONDS = 30.0
...
class CDCTestHelper:
    ...
    def start(self):
        ...
        if not self.ready_event.wait(timeout=CDC_CONSUMER_STARTUP_TIMEOUT_SECONDS):
        ...
        try:
            ...
            if isinstance(event_dict, dict) and "error" in event_dict:
                raise RuntimeError(
                    f"CDC consumer error: {event_dict['error']}\\n{event_dict.get('traceback', '')}"
                )
            else:
    ...
    def stop(self):
        ...
        if self.consumer_thread:
            self.stop_event.set()
            self.consumer_thread.join(timeout=CDC_CONSUMER_STOP_TIMEOUT_SECONDS)

    def wait_for_event(
        self, table: str, entity_id: str, timeout: float = CDC_EVENT_TIMEOUT_SECONDS
    ) -> CDCEvent:
        """
        ...
        while time.time() < deadline:
            try:
                ...
                seen_events.append(
                    f"{event.table}:{event.after.get('_id') if event.after else 'NO_ID'}"
                )
        ...
        raise TimeoutError(
            f"Timeout waiting for event: table={table}, entity_id={entity_id}. Seen: {seen_events}"
        )
...
def wait_for_database_ready(
    db_config: DatabaseConfig, timeout: float = CONTAINER_DB_READY_TIMEOUT_SECONDS
) -> None:
    """Wait until PostgreSQL accepts connections and queries."""
    deadline = time.time() + timeout
    last_error = "unknown"
    ...
    raise RuntimeError(f"PostgreSQL container was not ready within {timeout} seconds: {last_error}")
...
def ensure_container_ready(db_config):
    """Ensure PostgreSQL is ready before schema setup and test execution."""
...
@pytest.fixture(scope="module")
def config(setup_database, tmp_path_factory):
    """Create configuration from YAML string."""
    indented_sql_query = "\n".join(f"        {line}" for line in SQL_QUERY.strip("\n").splitlines())

    config_yaml = CONFIG_YAML_TEMPLATE.format(
        ...
        indented_sql_query=indented_sql_query,
    )
...
def cdc_helper(config, setup_database):
    ...
    # Create publication for tables under test
    publication_tables = ", ".join(TABLES_UNDER_TEST)
    with db_client.transaction() as cur:
        cur.execute(f"CREATE PUBLICATION {pub_name} FOR TABLE {publication_tables}")
    ...
    # Drain any initial events before each test begins
    helper.drain_queue()

    print("[FIXTURE] CDC helper ready")

    yield helper

    # Cleanup
    print("[FIXTURE] Stopping CDC consumer...")
    cleanup_warnings = []

    db_client = DatabaseClient(config.database.to_connection_params())

    # Terminate active replication backend first so consumer thread can exit cleanly.
    with db_client.transaction() as cur:
        cur.execute(
            """
    ...
    try:
        helper.stop()
        if helper.consumer_thread and helper.consumer_thread.is_alive():
            cleanup_warnings.append("CDC consumer thread did not stop cleanly before timeout")
    except Exception as exc:
        cleanup_warnings.append(f"Failed to stop CDC helper: {exc}")
    ...
    try:
        ...
        cleanup_warnings.append(f"Failed to drop replication slot {slot_name}: {e}")

    try:
        with db_client.transaction() as cur:
            cur.execute(f"DROP PUBLICATION IF EXISTS {pub_name}")
    except Exception as e:
        cleanup_warnings.append(f"Failed to drop publication {pub_name}: {e}")
...
def components(config):
    ...
    db_client = DatabaseClient(
        config.database.to_connection_params(), tracking_table=config.tracker.tracking_table
    )
...
class TestDepth0BaseTableChanges:
    ...
    def test_customer_insert(self, components, cdc_helper):
        ...
        event = cdc_helper.wait_for_event("customers", test_id, timeout=CDC_EVENT_TIMEOUT_SECONDS)
        ...
        with components["db_client"].transaction() as cur:
            cur.execute(
                "SELECT customer_id FROM customers_to_reprocess WHERE customer_id = %s", (test_id,)
            )
    ...
    def test_customer_update(self, components, cdc_helper):
        ...
        # Drain insert event
        cdc_helper.wait_for_event("customers", test_id, timeout=CDC_EVENT_TIMEOUT_SECONDS)

        # Update customer
        with components["db_client"].transaction() as cur:
            cur.execute("UPDATE customers SET name = %s WHERE _id = %s", ("Updated Name", test_id))
    ...
    def test_customer_delete(self, components, cdc_helper):
        ...
        # Soft delete
        with components["db_client"].transaction() as cur:
            cur.execute("UPDATE customers SET _deleted = TRUE WHERE _id = %s", (test_id,))
...
class TestDepth1ImmediateChanges:
    ...
    def test_order_insert(self, components, cdc_helper):
        ...
        cdc_helper.wait_for_event("customers", cust_id, timeout=CDC_EVENT_TIMEOUT_SECONDS)
        ...
        event = cdc_helper.wait_for_event("orders", order_id, timeout=CDC_EVENT_TIMEOUT_SECONDS)
        ...
        with components["db_client"].transaction() as cur:
            cur.execute(
                "SELECT customer_id FROM customers_to_reprocess WHERE customer_id = %s", (cust_id,)
            )
            assert cur.fetchone() is not None
    ...
    def test_order_update_join_key(self, components, cdc_helper):
        ...
        cdc_helper.wait_for_event("customers", cust1_id, timeout=CDC_EVENT_TIMEOUT_SECONDS)
        ...
        cdc_helper.wait_for_event("customers", cust2_id, timeout=CDC_EVENT_TIMEOUT_SECONDS)
        ...
        cdc_helper.wait_for_event("orders", order_id, timeout=CDC_EVENT_TIMEOUT_SECONDS)

        # Update order to customer 2
        with components["db_client"].transaction() as cur:
            cur.execute("UPDATE orders SET cust_id = %s WHERE _id = %s", (cust2_id, order_id))
        ...
        with components["db_client"].transaction() as cur:
            cur.execute(
                "SELECT customer_id FROM customers_to_reprocess WHERE customer_id = %s", (cust2_id,)
            )
            assert cur.fetchone() is not None
...
class TestDepth2AdaptiveChanges:
    ...
    def test_order_line_insert(self, components, cdc_helper):
        ...
        # Wait for event
        event = cdc_helper.wait_for_event("order_lines", ol_id, timeout=CDC_EVENT_TIMEOUT_SECONDS)
...
class TestDepth3DeferredChanges:
    ...
    def test_product_update_deferred(self, components, cdc_helper):
        ...
        cdc_helper.wait_for_event("products", prod_id, timeout=CDC_EVENT_TIMEOUT_SECONDS)

        # Update product price
        with components["db_client"].transaction() as cur:
            cur.execute("UPDATE products SET price = %s WHERE _id = %s", (25.99, prod_id))

        # Wait for event
        event = cdc_helper.wait_for_event("products", prod_id, timeout=CDC_EVENT_TIMEOUT_SECONDS)
...
class TestPercolation:
    ...
    def test_percolate_product_to_customer(self, components, cdc_helper):
        ...
        cdc_helper.wait_for_event("order_lines", ol_id, timeout=CDC_EVENT_TIMEOUT_SECONDS)
        ...
        with components["db_client"].transaction() as cur:
            cur.execute("UPDATE products SET price = %s WHERE _id = %s", (15.00, prod_id))
...
def test_summary(components, cdc_helper):
    ...
    # 1. Each test has an isolated CDC lifecycle
